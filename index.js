...
function logNames(items) {
  ...
  /**
   * @param {Item[]} items - arrclearay of items
   * @returns {string[]} an array of item names in all uppercase
   */
  function getUppercaseNames(items) {
    // TODO: use `map`
    const upperCaseName = items.map(item => item = item.name.toUpperCase());
  ...
  function countItems(items) {
    ...
    const totalQuatity = items.reduce((acc, cur) => acc + cur.quantity, initialValue);
    return totalQuatity;
  ...
  function calculateTotalPrice(items) {
    ...
    const totalCost = items.reduce((acc, cur) =>
      acc + ((cur.price) * (cur.quantity)), initialValue
    );
    return totalCost;
  }
